...
Amongst the boundary, we choose four stations "GA1321_B", "GA2401_D" for Hope Street, "GG2001_S", and "GA5371_C" for High Street.

```{r}
lez_monitor |> 
  filter(siteId %in% c("GA1321_B", "GA2401_D",   ## Near GLA4
...
st_distance(ap_glasgow |> st_transform(27700) |> filter(code == "GLA4"), 
            lez_monitor_for_ap |> filter(siteId %in% c("GA1321_B", "GA2401_D"))) 
...
traffic_tbl <- 
  ...
  mutate(location = if_else(siteId == "GA1321_B" | siteId == "GA2401_D", "Hope St", "High St"))
...
df_for_analysis |> 
  ...
  mutate(dt_month = if_else(dt_month == "6", "June", "August"),
         dt_month = factor(dt_month, levels = c("June", "August"))) |> 
  ggplot(aes(dt_date, dailyflow, colour = siteId)) +
...
ggsave("Figures/average daily traffic flow.jpg", width = 9, height = 3.5)
...
```{r}
df_for_analysis |> 
  mutate(siteId = case_when(siteId == "GA1321_B" ~ "GA1321_B (Hope St)",
...
no2_2023 |> 
  ...
  filter(dt_month == 6 | dt_month == 8) |> 
  #filter(dt_month >= "2023-06-01" & dt_date <= "2023-08-31") |> 
...
ggsave("no2 by year.jpg", width = 5, height = 4)
...
ggsave("no2 by station.jpg", width = 8.5, height = 4)
